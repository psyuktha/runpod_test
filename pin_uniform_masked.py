#!/usr/bin/env python3
"""
Mask the center, detect pins, classify by sides using edge strips, and annotate.

Defaults:
  min_area=120, max_area=800, min_side=3, threshold=128, center_mask_ratio=0.4
  side strip = 7% of smaller package dimension (min 3 px)

Usage (from repo root):
  python pin_uniform_masked.py bi_full/bilateral_canny_anu4.jpg \
      --out-image bi_full/anu4_uniform_masked.png
"""
...
def load_mask_with_center(path: Path, threshold: int, center_mask_ratio: float) -> np.ndarray:
    arr = np.array(Image.open(path).convert("L"))
    mask = (arr > threshold).astype(np.uint8)
    h, w = mask.shape
    cx, cy = w // 2, h // 2
    mw = int(w * center_mask_ratio)
    mh = int(h * center_mask_ratio)
    x0 = max(0, cx - mw // 2)
    x1 = min(w, cx + mw // 2)
    y0 = max(0, cy - mh // 2)
    y1 = min(h, cy + mh // 2)
        ...
        for x in range(w):
            ...
            while q:
                ...
                for dy, dx in neighbors:
                    ny, nx = cy + dy, cx + dx
                    if 0 <= ny < h and 0 <= nx < w and mask[ny, nx] == 1 and labels[ny, nx] == 0:
                        labels[ny, nx] = label
                        q.append((ny, nx))
            areas.append(area)
...
def side_for_pin(pin_bbox: Tuple[int, int, int, int], pkg_bbox: Tuple[int, int, int, int], strip_ratio: float = 0.07) -> str:
    ...
    if cx >= px_max - strip:
        return "right"
    dists = {"top": abs(cy - py_min), "bottom": abs(py_max - cy), "left": abs(cx - px_min), "right": abs(px_max - cx)}
...
def spacing_score(centers: List[Tuple[float, float]], side: str) -> float:
    if len(centers) < 2:
        return float("inf")
    coords = sorted(c[0] for c in centers) if side in ("top", "bottom") else sorted(c[1] for c in centers)
...
def analyze(
    ...
    mask = load_mask_with_center(path, threshold, center_mask_ratio)
    ...
    side_counts: Dict[str, int] = {k: 0 for k in ("top", "bottom", "left", "right", "unknown")}
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Mask center, annotate pins, count per side, pick best side by spacing.")
    ...
    parser.add_argument("--center-mask-ratio", type=float, default=0.4, help="Fraction of image width/height to mask at center")
    ...
    side_counts, side_labels, side_scores, best_side, symmetric_estimate, pins_bboxes, pins_labels, pkg_bbox = analyze(
        args.image, args.min_area, args.max_area, args.min_side, args.threshold, args.center_mask_ratio
    )
